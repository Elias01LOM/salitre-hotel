...
$pageTitle = 'Hotel Salitre';
$extraCss = '../../assets/css/client/index.css';
...
<main>
  <section id="hero" class="hero">
    <video class="hero__video" autoplay muted loop playsinline aria-hidden="true">
      <source src="<?= htmlspecialchars(salitre_url('assets/video/hero-bg.mp4'), ENT_QUOTES, 'UTF-8') ?>" type="video/mp4">
    </video>
    ...
    <div class="hero__content">
      ...
      <div class="hero__actions">
        <a class="btn btn-primary" href="espacios/" aria-label="Ver espacios disponibles">Ver Espacios</a>
        ...
        if (preg_match('/\\.webp$/i', $fotoWebp)) {
          $fotoJpg = preg_replace('/\\.webp$/i', '.jpg', $fotoWebp);
            ...
            $fotoWebp = preg_replace('/\\.jpe?g$/i', '.webp', $fotoWebp);
              ...
              // Si no trae extensión conocida, se usa tal cual como WebP.
              $fotoJpg = $fotoWebp;
              }
              }
              $precioFormateado = number_format($precio, 0, ',', '.');
              $detalleUrl = 'espacios/detalle.php?slug=' . rawurlencode($slug);
              ?>
              <article class="space-card">
                <div class="space-card__media">
                  <?php if (!empty($fotoWebp) && !empty($fotoJpg)): ?>
                    <picture>
                  <source
                    srcset="<?= htmlspecialchars(salitre_url($fotoWebp), ENT_QUOTES, 'UTF-8') ?>"
                    type="image/webp"
                  >
                  <img
                    src="<?= htmlspecialchars(salitre_url($fotoJpg), ENT_QUOTES, 'UTF-8') ?>"
    ...
    <div class="index-container">
      <div class="contacto__grid">
        <div class="contacto__left">
          ...
          <form
            class="contact-form"
            action="<?= htmlspecialchars(salitre_url('client/includes/procesar_contacto.php'), ENT_QUOTES, 'UTF-8') ?>"
            method="post"
          >
            <label class="field">
